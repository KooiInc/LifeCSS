...
function TOAFactory() {
  ...
  function getResult(input, compareWith, noShouldbe, me) {
    ...
    if (input?.[Symbol.proxy] && compareWith === Proxy) {
      return compareWith === Proxy;
    }
    if (maybe2({ trial: (_) => String(compareWith) }) === `NaN`) {
      return String(input) === `NaN`;
    }
    if (input?.[Symbol.toStringTag] && IS2(compareWith, String)) {
...
// ../index.js
var LifeCSS_default = LifeStyleFactory;
